...
cdef class ReactorOde(object):
    ...
    cdef float T_onset, T1,T2,dTmax
...
cpdef main(parameters):
    ...
    if loglevel:
        ...
        """
        #------------------------------------------------------
        with open(os.path.join(path_output,'rates_of_evaporating_species.txt'),'w') as File:
            #File.writelines(header)
    ...
    while solver.successful() and solver.t < t_end:    
        ...
        if solver.t%2<dt:
            ...
            if loglevel:    
                ...
                """
                with open(path2output+'rates_of_evaporating_species.txt','a') as File:
                    File.writelines('%-10s %-5.3f %15s %15s %15s %15s\n' % ('Temp(C)=',liquid.T-273.0, 'Y_liquid', 'wdot_chem', 'wdot_evap', 'klg'))
    ...
    with open(path2output+'rates_of_production.txt','a') as File:
        File.writelines('%15.3f\n' % solver.t)
        for indx in np.absolute(liquid.net_rates_of_production).argsort()[-10:][::-1]:
            File.writelines('%20s %15.3E' %(liquid.species_name(indx),liquid.net_rates_of_production[indx]))
            File.writelines("\n")
